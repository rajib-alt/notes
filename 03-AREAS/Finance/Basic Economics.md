...
mindmap
  root((Basic Economics))
    ...
    Profits & Losses
      Feedback mechanism
      Losses = stop signal
      Bail-outs break the system
    Wages & Value
      Pay = value created
      Minimum wage paradox
      Maria vs Jake example
    Trade
      Both sides win
      Comparative advantage
      Specialisation = wealth
```
...
```mermaid
flowchart LR
    A["Government raises<br>millionaire tax"] --> B["Intended: +$106M revenue"]
    A --> C["Actual: 2,000 millionaires leave"]
    C --> D["Businesses & jobs disappear"]
    D --> E["Lost: $257M in revenue"]
...
```mermaid
flowchart TD
    A["$50,000 available"] --> B{Choice}

    B --> C["Buy BMW<br/>$50,000"]
    B --> D["Buy Toyota Corolla<br/>$20,000<br/>+ Invest $30,000"]

    C --> E["😎 Leather seats<br/>Feels like success"]
    D --> F["📈 $30K invested<br/>→ ~$60K in 10 years<br/>→ ~$250K in 30 years"]

    E --> G["Opportunity cost:<br/>~$250,000 in forgone wealth"]
...
sequenceDiagram
    ...
    Consumer->>Price: High demand for pizza
    Price->>Supplier: Price rises — signal: "Make more!"
    Supplier->>Consumer: More pizza produced, supply increases
    Consumer->>Price: Demand drops, too much pizza
    Price->>Supplier: Price falls — signal: "Slow down"
    Supplier->>Consumer: Production adjusts down
```
...
```mermaid
flowchart TD
    A["Government Sets<br>Price Controls"] --> B{Direction}
    B --> C["Price set TOO LOW<br>(e.g. rent control)"]
    B --> D["Price set TOO HIGH<br>(e.g. wheat price floor)"]
    C --> E["Demand explodes<br>Supply collapses<br>SHORTAGE"]
    D --> F["Supply explodes<br>Demand can't absorb it<br>SURPLUS → Rot"]
    E --> G["NYC: Empty apartments<br>+ homeless on street"]
    E --> H["Zimbabwe: Empty shelves<br>farmers stopped selling"]
    F --> I["India 2000s: 11M tons<br>of wheat rotted in warehouses"]
    style E fill:#f87171,color:#000
...
```mermaid
flowchart LR
    A["Company produces<br>something"] --> B{Market Feedback}
    B -- "People buy it" --> C["PROFIT<br>→ Signal: do more"]
    B -- "Nobody wants it" --> D["LOSS<br>→ Signal: stop, redirect"]
    C --> E["Resources flow toward<br>valuable production"]
    D --> F["Resources freed up<br>for better use"]
    
    G["Government Bailout"] --> H["Overrides loss signal"]
    H --> I["Company keeps wasting<br>resources on unwanted output<br>funded by taxes"]
...
```mermaid
flowchart TD
    A["Lemonade Stand<br>Earns $1/cup"] --> B{Hire worker?}
    B --> C["Maria:<br>Friendly & fast<br>→ 10 extra cups = $10 value"]
    B --> D["Jake:<br>Slow, on phone<br>→ 2 extra cups = $2 value"]
    C --> E["Can pay up to $10/hr<br>and still profit"]
    D --> F["Can only pay $2/hr<br>or lose money"]
    style E fill:#4ade80,color:#000
    style F fill:#f87171,color:#000
```
...
```mermaid
flowchart LR
    A["You have: Sandwich<br>Want: Pizza"] --> C{Trade}
    B["Friend has: Pizza<br>Wants: Sandwich"] --> C
    C --> D["You get half pizza 🍕"]
    C --> E["Friend gets half sandwich 🥪"]
    D --> F["Both are happier<br>No extra food created<br>Value appeared from nowhere"]
    E --> F
    style F fill:#4ade80,color:#000
```
...
```mermaid
flowchart TD  
L1["Law 1 — Incentives Rule<br/>People respond to rewards<br/>not intentions"] --> L2  
L2["Law 2 — Opportunity Cost<br/>Every choice sacrifices<br/>the next best option"] --> L3  
L3["Law 3 — Prices Signal<br/>Prices carry more information<br/>than any planner can"] --> L4  
L4["Law 4 — Price Controls Fail<br/>Cap prices → shortage<br/>Floor prices → surplus"] --> L5  
L5["Law 5 — Profits & Losses Direct<br/>Profit = do more<br/>Loss = stop, redirect"] --> L6  
L6["Law 6 — Wages Reflect Market Value<br/>Pay reflects results<br/>not just effort or time"] --> L7  
L7["Law 7 — Trade Creates Wealth<br/>Both parties win<br/>Specialization multiplies output"]
```
