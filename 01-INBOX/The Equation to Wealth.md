...
flowchart TD
    A[Time] --> B[Job / Salary]
    B --> C[Fixed Income]
    C --> D[Slow Wealth Accumulation]
    D --> E[Retirement — Most Time Gone]
```

![Salary vs wealth accumulation](https://source.unsplash.com/800x400/?money,time,clock)
...
```mermaid
flowchart TD
    A[Observe the Market] --> B[Identify a Real Problem]
    B --> C[Find or Build a Solution]
    C --> D[Package as a Business]
    D --> E[Deliver Value to the Market]
    E --> F[Market Rewards You with Money]
```

![Problem solving and entrepreneurship](https://source.unsplash.com/800x400/?entrepreneur,problem,solution)
...
```mermaid
flowchart LR
    A[Problem Identified] --> B[Solution Built]
    B --> C[Business Scaled]
    C --> D{Decision Point}
    D --> E[Sell / Acquire]
    D --> F[Continue Running]
    E --> G[Wealth Event]
    F --> G
```

![Business acquisition and exit](https://source.unsplash.com/800x400/?business,deal,handshake)
